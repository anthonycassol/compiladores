<?php
	...
	function lexico($linha, $valorlinha, $afd, $tabSimb){
		...
		$length = strlen($linha);
		//percorre todos os simbolos da linha
		$vetor = array();
		$palavra = "";
		for ($i=0; $i<$length; $i++) {
			$simbolo = $linha[$i];
	...
	function verificalexico($vetor, $valorlinha, $afd, $tabSimb, $palavra){
		...
		foreach ($vetor as $vet => $letra) {
			...
			if(in_array($letra, $tabSimb)){
				$chave = array_search($letra, $tabSimb)+1;
			}
			else{
	...
	function getParsing($xml, $parsing){
		...
		foreach($xml->m_Symbol->Symbol as $value) {
			$index = xml_attribute($value, 'Index');
			$simbolo = xml_attribute($value, 'Name');
			$GLOBALS['parserSimbolos'][$index] = $simbolo;
		}

		foreach($xml->LALRTable->LALRState as $i) {

			$linha = xml_attribute($i, 'Index');
			foreach($i->LALRAction as $j){
				$aux = new LALRState; 
				$aux->estado = xml_attribute($j, 'Value');
				$aux->acao = xml_attribute($j, 'Action');
	...
	function sintatico($parsing, $fita, $xml){
		...
		while($erro != 1 AND $acao != 4){
			...
			if(isset($parsing[$ultimo][$simbolo])){
